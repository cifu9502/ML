...
lamb = 0.001
reg =  ((lamb*0.5)*(tf.reduce_sum(tf.multiply(W1,W1))+tf.reduce_sum(tf.multiply(W2,W2)))) 
cross_entropy = tf.reduce_mean(-tf.reduce_sum( tf.one_hot(yl,depth=K) * tf.log(a2+eps) +  (1.0-tf.one_hot(yl,depth=K) )*tf.log(1.0-a2 +eps) , reduction_indices=[1]))+reg # a little more explicit
