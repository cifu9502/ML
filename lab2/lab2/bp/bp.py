...
lamb = 0.002 ##* (np.random.randn(1)+1) # Second layer weight matrix
print(lamb)
...
for i in range(20000):
  ...
  eps=0.00000001
  loss = -np.mean(np.sum( oneh*np.log(a2+eps) + (1.0-oneh)*np.log(1.0-a2+eps),axis=1 )) + ((lamb*0.5)*(np.sum(np.multiply(W1,W1))+np.sum(np.multiply(W2,W2))))
...
Z = sigmoid(np.dot(lineSigmoid( np.dot(np.c_[xx.ravel(), yy.ravel()], W1) + b1), W2) + b2)
